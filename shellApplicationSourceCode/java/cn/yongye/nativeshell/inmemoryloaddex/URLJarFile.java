...
//                            Path tmpFile = Files.createTempFile("jar_cache", null);
                            File tmpFile = new File("jar_cache");
                            if(!tmpFile.exists())
...
//                                tmpFile.toFile().deleteOnExit();
                                ...
                                int inReadNumb = 0;
                                while((inReadNumb = in.read(btTmp, 0, btTmp.length)) != -1){
                                    fout.write(btTmp, 0, inReadNumb);
                                }
                                fout.close();
                                JarFile jarFile = new URLJarFile(tmpFile, closeController);
                                return jarFile;
                            } catch (Throwable thr) {
                                tmpFile.delete();
//                                try {
