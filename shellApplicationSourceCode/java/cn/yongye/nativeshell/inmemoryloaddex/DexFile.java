...
package cn.yongye.nativeshell.inmemoryloaddex;
...
public final class DexFile {
    public static final String TAG = "永夜" ;
  /**
...
//            return "InMemoryDexFile[cookie=" + Arrays.toString((long[]) mCookie) + "]";
            return "InMemoryDexFile[cookie=" + mCookie + "]";
        }
    }
...
//            result = defineClassNative(name, loader, cookie, dexFile);
    ...
    private static class DFEnum implements Enumeration<String> {
        ...
        DFEnum(DexFile df) {
            mIndex = 0;
            try {
                Class clsgetClassNameList = Class.forName("dalvik.system.DexFile");
                Method mdgetClassNameList = clsgetClassNameList.getMethod("getClassNameList", long.class);
                mdgetClassNameList.invoke(null, df.mCookie);
            } catch (ClassNotFoundException e) {
                e.printStackTrace();
            } catch (NoSuchMethodException e) {
                e.printStackTrace();
...
//    }
    ...
    static {
        System.loadLibrary("dalvik_system_DexFile");
    }

    private static long openInMemoryDexFile(ByteBuffer buf) throws IOException {
        // if (buf.isDirect()) {
        //     return createCookieWithDirectBuffer(buf, buf.position(), buf.limit());
        // } else {
        //     long ret = createCookieWithArray(buf.array(), buf.position(), buf.limit());
        //     return ret;
        // }
        return createCookieWithArray(buf.array(), buf.position(), buf.limit());
    }

    private static native long createCookieWithDirectBuffer(ByteBuffer buf, int start, int end);
    private static native long createCookieWithArray(byte[] buf, int start, int end);
